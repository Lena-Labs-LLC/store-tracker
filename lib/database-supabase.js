...
class SupabaseDatabase {
    ...
    async addStore(name, url, type, checkInterval = 24, intervalUnit = 'hours', studioTag = null) {
        ...
        let insertData = {
            name,
            studio_tag: this.normalizeStudioTag(studioTag),
            url,
            type,
        ...
        if (error) {
            ...
            if (error.message.includes('check_interval_unit') || error.message.includes('check_interval_value') || error.message.includes('studio_tag')) {
                ...
                if (studioTag) {
                    try {
                        await this.updateStoreTag(legacyData.id, studioTag);
                    } catch (tagError) {
                        console.log('Studio tag column not available yet, skipping tag update');
                    }
                }
    ...
    async updateStoreTag(id, studioTag = null) {
        const { error } = await this.supabase
            .from('stores')
            .update({ studio_tag: this.normalizeStudioTag(studioTag) })
            .eq('id', id)
